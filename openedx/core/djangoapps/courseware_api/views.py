...
class Celebration(DeveloperErrorViewMixin, APIView):
    ...
    **Returns**
    ...
    def post(self, request, course_key_string, *args, **kwargs):
        ...
        # Intentionally don't allow masquerading here (doesn't make sense to turn off someone else's celebration)
        target_user = self.request.GET.get('username')
        if self.request.user.is_staff and target_user and target_user != self.request.user.username:
